...
internal static class SettingsHelper
{
    ...
    {
        Rect fullRect = ls.GetRect(height: Text.LineHeight);
        Rect descLabelRect = fullRect.LeftPart(pct: .50f).Rounded();
        Rect sliderRect = fullRect.RightPart(pct: .50f).Rounded().LeftPart(pct: .90f).Rounded();
        Rect percentLabelRect = fullRect.RightPart(pct: .10f).Rounded();
        ...
        {
            TooltipHandler.TipRegion(rect: fullRect, tip: tooltip);
        }
